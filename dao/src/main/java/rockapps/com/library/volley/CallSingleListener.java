...
import rockapps.com.library.volley.mymodels.JsonMessageSingle;

public class CallSingleListener<E> extends CallListener<E, JsonMessageSingle<E>> {
    ...
    public CallSingleListener(Context context, Class<E> type, Boolean saveLocal, String saveLocalName) {
    ...
    public CallSingleListener(Activity activity, Class<E> type, RelativeLayout parentLayout, OnDialogButtonClick onDialogButtonClick) {
    ...
    public void onParse(JSONObject json) {
        jsonMessage = gson.fromJson(json.toString(), JsonMessageSingle.class);
        try {
            jsonMessage.setObject(getObject(type, json.getJSONObject(objectName)));
            if (saveLocal) {
                saveLocal(jsonMessage.getObject(), saveLocalName);
            }
